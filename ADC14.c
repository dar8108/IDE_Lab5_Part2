...
void ADC0_InitSWTriggerCh6(void)
{
	...
	// REF_A->CTL0
    while ((REF_A->CTL0 & BIT(10)) != 0)
    {
        // Wait until REFGENBUSY is 0 (not busy)
    }        
    ...
    {
        // Wait until REFGENRDY is 1 (ready)
    }
  ...
  // ------------------------------------------------------------------
    ...
    // 0000 0100 0010 0000 0011 0011 0001 0000
    ADC14->CTL0 = ADC14CTL0_INIT;	
...
unsigned int ADC_In(void)
{
	...
	// ADC14->MEM[0] 14-bit conversion in bits 13-0 (31-16 undefined, 15-14 zero)
	// Mask off bits 31 to 14
	adcIn = ADC14->MEM[0] & 0x00003FFF; 
